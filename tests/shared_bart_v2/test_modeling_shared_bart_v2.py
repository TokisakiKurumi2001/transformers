...
""" Testing suite for the PyTorch SHARED_BART_V2 model. """
...
from transformers import BartSharedV2Config, is_torch_available
...
if is_torch_available():
    ...
    from transformers.models.shared_bart_v2.modeling_shared_bart_v2 import BartSharedV2Decoder, BartSharedV2Encoder, shift_tokens_right


def prepare_shared_bart_v2_inputs_dict(
    config,
    input_ids,
...
class BartSharedV2ModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        inputs_dict = prepare_shared_bart_v2_inputs_dict(config, input_ids, decoder_input_ids)
        return config, inputs_dict

    def get_config(self):
        return BartSharedV2Config(
    ...
    def create_and_check_decoder_model_past_large_inputs(self, config, inputs_dict):
        model = BartSharedV2Model(config=config).get_decoder().to(torch_device).eval()
    ...
    def check_encoder_decoder_model_standalone(self, config, inputs_dict):
        model = BartSharedV2Model(config=config).to(torch_device).eval()
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            encoder = BartSharedV2Encoder.from_pretrained(tmpdirname).to(torch_device)
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            decoder = BartSharedV2Decoder.from_pretrained(tmpdirname).to(torch_device)
...
@require_torch
class BartSharedV2HeadTests(unittest.TestCase):
    vocab_size = 99
    ...
    def test_sequence_classification_forward(self):
        ...
        model = BartSharedV2ForSequenceClassification(config)
    ...
    def test_question_answering_forward(self):
        ...
        model = BartSharedV2ForQuestionAnswering(config)
        model.to(torch_device)
        outputs = model(
            input_ids=input_ids,
    ...
    def test_lm_forward(self):
        ...
        lm_model = BartSharedV2ForConditionalGeneration(config)
    ...
    def test_lm_uneven_forward(self):
        ...
        lm_model = BartSharedV2ForConditionalGeneration(config).to(torch_device)
    ...
    def test_tokenization(self):
        tokenizer = BartTokenizer.from_pretrained("facebook/shared_bart_v2-large")
        ...
        for ex, desired_result in zip(examples, fairseq_results):
            shared_bart_v2_toks = tokenizer.encode(ex, return_tensors="pt").squeeze()
            assert_tensors_close(desired_result.long(), shared_bart_v2_toks, prefix=ex)

    def test_generate_fp16(self):
        config, input_ids, batch_size = self._get_config_and_data()
        attention_mask = input_ids.ne(1).to(torch_device)
        model = BartSharedV2ForConditionalGeneration(config).eval().to(torch_device)
...
@require_torch
class BartSharedV2ModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (
        (BartSharedV2Model, BartSharedV2ForConditionalGeneration, BartSharedV2ForSequenceClassification, BartSharedV2ForQuestionAnswering)
        if is_torch_available()
        else ()
    )
    all_generative_model_classes = (BartSharedV2ForConditionalGeneration,) if is_torch_available() else ()
    is_encoder_decoder = True
    test_pruning = False
    test_missing_keys = False

    def setUp(self):
        self.model_tester = BartSharedV2ModelTester(self)
        self.config_tester = ConfigTester(self, config_class=BartSharedV2Config)
    ...
    # BartSharedV2ForSequenceClassification does not support inputs_embeds
    def test_inputs_embeds(self):
        config, inputs_dict = self.model_tester.prepare_config_and_inputs_for_common()

        for model_class in (BartSharedV2Model, BartSharedV2ForConditionalGeneration, BartSharedV2ForQuestionAnswering):
            model = model_class(config)
            model.to(torch_device)
            model.eval()
...
class FastIntegrationTests(unittest.TestCase):
    ...
    @cached_property
    def tok(self):
        return BartTokenizer.from_pretrained("facebook/shared_bart_v2-large")

    @cached_property
    def xsum_1_1_model(self):
        return BartSharedV2ForConditionalGeneration.from_pretrained("sshleifer/distilshared_bart_v2-xsum-1-1")
...
@require_tokenizers
class BartSharedV2ModelIntegrationTests(unittest.TestCase):
    @cached_property
    ...
    def test_inference_no_head(self):
        model = BartSharedV2Model.from_pretrained("facebook/shared_bart_v2-large").to(torch_device)
    ...
    def test_base_mask_filling(self):
        pbase = pipeline(task="fill-mask", model="transZ/bart_shared_v2")
    ...
    def test_large_mask_filling(self):
        plarge = pipeline(task="fill-mask", model="facebook/shared_bart_v2-large")
        src_text = [" I went to the <mask>."]
    ...
    def test_mnli_inference(self):
        ...
        model = AutoModelForSequenceClassification.from_pretrained("facebook/shared_bart_v2-large-mnli").to(
            torch_device
    ...
    def test_xsum_summarization_same_as_fairseq(self):
        model = BartSharedV2ForConditionalGeneration.from_pretrained("facebook/shared_bart_v2-large-xsum").to(torch_device)
    ...
    def test_xsum_config_generation_params(self):
        config = BartSharedV2Config.from_pretrained("facebook/shared_bart_v2-large-xsum")
    ...
    def test_cnn_summarization_same_as_fairseq(self):
        hf = BartSharedV2ForConditionalGeneration.from_pretrained("facebook/shared_bart_v2-large-cnn").to(torch_device)
        tok = BartTokenizer.from_pretrained("facebook/shared_bart_v2-large")
...
class BartSharedV2StandaloneDecoderModelTester:
    def __init__(
        self,
        parent,
        vocab_size=99,
    ...
    def create_and_check_decoder_model_past(
        ...
        model = BartSharedV2Decoder(config=config).to(torch_device).eval()
        # first forward pass
...
@require_torch
class BartSharedV2StandaloneDecoderModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (BartSharedV2Decoder, BartSharedV2ForCausalLM) if is_torch_available() else ()
    all_generative_model_classes = (BartSharedV2ForCausalLM,) if is_torch_available() else ()
    test_pruning = False
    is_encoder_decoder = False
    ...
    def setUp(
        ...
        self.model_tester = BartSharedV2StandaloneDecoderModelTester(self, is_training=False)
        self.config_tester = ConfigTester(self, config_class=BartSharedV2Config)
